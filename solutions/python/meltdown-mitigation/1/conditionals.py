...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    """
# Calculate the generated power first
    generated_power = voltage * current
    
    # Calculate efficiency percentage
    efficiency = (generated_power / theoretical_max_power) * 100

    # Chained conditional checks based on the bands
    if efficiency >= 80:
        return 'green'
    elif efficiency >= 60:
        return 'orange'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if product < (0.9 * threshold):
        ...
        # Because we already checked for 'LOW', we know this is 
        # at least 90% and up to 110% of the threshold.
        return 'NORMAL'
    else:
        # Everything else is considered above the 10% threshold.
        return 'DANGER'